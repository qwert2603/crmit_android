...
class StudentDetailsFragment : EntityDetailsFragment<StudentFull>() {
    ...
    override fun StudentFull.toDetailsList(): List<EntityDetailsListItem> = listOfNotNull(
            ...
            EntityDetailsField(R.string.student_details_field_login, systemUser.login, R.drawable.ic_person_black_24dp),
            EntityDetailsField(R.string.student_details_field_lessonsAttendedCount, lessonsAttendedCount.toString()),
            EntityDetailsField(R.string.student_details_field_birthDate, showingBirthDate()),
            EntityDetailsField(R.string.student_details_field_birthPlace, birthPlace),
            EntityDetailsField(R.string.student_details_field_registrationPlace, registrationPlace),
            EntityDetailsField(R.string.student_details_field_actualAddress, actualAddress),
            additionalInfo?.takeIf { it.isNotEmpty() }?.let { EntityDetailsField(R.string.student_details_field_additionalInfo, it) },
            knownFrom?.takeIf { it.isNotEmpty() }?.let { EntityDetailsField(R.string.student_details_field_knownFrom, it) },
            EntityDetailsField(R.string.student_details_field_school, getString(R.string.student_school_format, school.name, grade, shift)),
            phone?.let { EntityDetailsField(R.string.student_details_field_phone, it, R.drawable.ic_local_phone_black_24dp) },
            EntityDetailsField(R.string.student_details_field_contactPhone, "${contactPhoneNumber} (${contactPhoneWho})", R.drawable.ic_local_phone_black_24dp),
            EntityDetailsField(R.string.student_details_field_citizenshipName, citizenshipName),
            mother?.let { EntityDetailsField(R.string.student_details_field_mother, it.toTextFieldValue()) },
            father?.let { EntityDetailsField(R.string.student_details_field_father, it.toTextFieldValue()) },
